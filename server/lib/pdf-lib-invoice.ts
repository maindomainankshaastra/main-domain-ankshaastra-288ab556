...
export async function generateInvoicePdfWithPdfLib(data: InvoiceTemplateData): Promise<Buffer> {
  ...
  // Order ID — printed on its own line under Invoice #/Date/Due Date so an
  // invoice can always be traced back to the order it was generated from.
  // This is the pdf-lib fallback renderer (used when Puppeteer/Chromium
  // isn't available, e.g. on Vercel serverless) — it does NOT go through
  // renderInvoiceHtml/invoice-html.ts at all, so Order ID has to be drawn
  // here separately too.
  if (data.orderId) {
    page.drawText('Order ID', { x: margin, y, size: 8, font: fontBold, color: gray });
    page.drawText(sanitizePdfText(data.orderId), { x: margin + 60, y, size: 8, font, color: black });
    y -= 16;
  }
