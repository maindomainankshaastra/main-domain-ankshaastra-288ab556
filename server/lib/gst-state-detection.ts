...
// //   const currentCityRaw = pickString(snapshot, ['currentCity']);
// //   if (currentCityRaw) {
// //     const fromCity = parseStateFromAddress(currentCityRaw);
// //     if (fromCity.stateCode) {
// //       return {
// //         stateName: fromCity.stateName || fromCity.stateCode,
// //         stateCode: fromCity.stateCode,
// //         source: 'current_city',
// //       };
// //     }
// //   }
...
//   order?: Row | null,
// ): { stateName: string; stateCode: string; source: string } {
//   // FIX: `metadata`/`snapshot` moved up here — previously declared further
//   // down but referenced earlier (in the `currentCityRaw` block below),
//   // which threw "Cannot access 'snapshot' before initialization" (TDZ
//   // error on a `const`) whenever billing_state/billing_address didn't
//   // resolve a state. That crashed invoice generation entirely instead of
//   // falling through to the remaining detection sources.
//   const metadata = (order?.metadata as Row | undefined) || {};
//   const snapshot = (metadata.formSnapshot as Row | undefined) || metadata;
...
import { stateCodeFromName, stateNameFromCode } from './indian-states.js';
import { GST_COMPANY_DEFAULTS, UNKNOWN_STATE_CODE, UNKNOWN_STATE_NAME } from './gst-company-defaults.js';
...
export function detectCustomerState(
  ...
  // Client requirement: when no state can be detected from any source
  // (billing address, order snapshot, profile, etc.), default to the
  // company's own registered state (09 / Uttar Pradesh) instead of
  // leaving it "Unknown (00)". Note: this makes such invoices
  // intra-state (CGST+SGST) since the company is also registered in UP —
  // confirmed with client as the intended behavior.
  return {
    stateName: GST_COMPANY_DEFAULTS.registeredState,
    stateCode: GST_COMPANY_DEFAULTS.stateCode,
    source: 'default_up',
  };
}