...
// FIX: now also accepts PARTNER_API_KEY, in addition to the existing
// OPERATIONS_API_KEYS list. This is purely additive — every key that
// worked before still works — it just adds a second, separately-issued
// key for the new /operations/trigger-invoice endpoint used by Empower
// and Miracle Baby.
export function verifyApiKey(provided: string | undefined): boolean {
  const rawKeys = [process.env.OPERATIONS_API_KEYS, process.env.PARTNER_API_KEY]
    .filter(Boolean)
    .join(",");
