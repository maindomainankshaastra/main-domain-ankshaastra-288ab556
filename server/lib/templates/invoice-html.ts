...
//   // Dynamic, service-specific form fields — either an exact per-service
//   // template (with section headers, e.g. "Person 1 Details") or the
//   // generic auto-formatted fallback for services without one yet.
//   // See getOrderFormRows() / FormRow in order-form-details.ts.
//   serviceFormFields?: import('../order-form-details.js').FormRow[];
...
// // self-order (the customer bought the service for themselves), where
// // "Service Subject" would just repeat "Purchased By" and add nothing.
...
//   // Only show a subject if it's someone OTHER than the purchaser — e.g. a
//   // parent ordering a baby-name check for their child, or a Name Check done
//   // for a family member. When the order is for the purchaser themselves
//   // (the common case), the subject name is identical to "Purchased By" and
//   // showing it again as "Service Subject: <same name>" is pure duplication,
//   // which is what was happening before this filter.
//   const subjects = (data.serviceSubjects || []).filter((s) => !namesMatch(s.full_name, purchasedBy));
...
export type InvoiceTemplateData = {
  ...
  invoiceDate: string;
  dueDate: string;
  // The order this invoice was generated from — printed on the PDF next
  // to Invoice # so customers/support can always trace an invoice back
  // to its order.
  orderId?: string;
  businessName: string;
  businessGstin?: string;
...
<div class="page">
  ...
  <div class="meta-grid">
    ...
    <div><strong>Invoice Date</strong>${escape(data.invoiceDate)}</div>
    <div><strong>Due Date</strong>${escape(data.dueDate)}</div>
    ${data.orderId ? `<div><strong>Order ID</strong>${escape(data.orderId)}</div>` : ''}
  </div>
