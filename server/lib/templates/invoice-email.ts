...
// FIX (per client request 2026-08-01): the email used to repeat the full
// GST breakdown (Taxable Amount, CGST/SGST/IGST, Total Paid) as its own
// table — that detail already lives in the attached PDF, so it was
// redundant. The email body is now a short, simple message (matching the
// admin's manual "resend" email style) — the structured Order Information /
// Customer Contact / Order Details sections are provided separately via
// orderDetailsHtml (see order-form-details.ts).
export function buildInvoicePaymentEmailHtml(input: {
  customerName: string;
  serviceTitle: string;
  invoiceNumber: string;
