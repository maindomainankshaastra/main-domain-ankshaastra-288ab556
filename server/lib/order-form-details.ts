...
function sectionTable(rows: FormRow[]): string {
  ...
  return rows
    .map((row) =>
      row.kind === 'section'
        ? `<div style="margin:8px 0 2px;line-height:1.3;font-size:13px;font-weight:700;color:#4b77be;">${escapeHtml(row.title)}</div>`
        : `<div style="margin:0 0 2px;line-height:1.3;font-size:14px;"><strong style="color:#444;">${escapeHtml(row.label)}:</strong> ${escapeHtml(row.value)}</div>`,
    )
    .join('');
}

function sectionHeading(title: string): string {
  return `<div style="margin:10px 0 4px;line-height:1.3;font-size:15px;font-weight:700;color:#4b77be;border-bottom:1px solid #e5e7eb;padding-bottom:4px;">${escapeHtml(title)}</div>`;
}
...
const SERVICE_TEMPLATES: Record<string, (snapshot: Record<string, unknown>) => FormRow[]> = {
  ...
  // FIX (2026-08-09): Name Check for 3 people ("namecheck-3") is a real,
  // live Empower package (confirmed in ankshaastra-empower's own
  // packagePricing.ts/OrderFormSection.tsx — it collects a full
  // person3FirstName/person3MiddleName/person3SurName/person3Dob/
  // person3Gender/person3MiddleNameType set, same shape as person1/person2
  // above) but had no template registered here at all. Any 3-person Name
  // Check order was silently falling through to the generic auto-formatter
  // — the exact same failure mode as the "premium" Baby Name Report bug.
  'empower.ankshaastra.com::namecheck-3': (s) => [
    section('Person 1 Details'),
    field('First Name', s.person1FirstName, 'person1FirstName'),
    field('Middle Name', s.person1MiddleName, 'person1MiddleName'),
    field('Last Name', s.person1SurName, 'person1SurName'),
    ...
    field('Date of Birth', s.person2Dob, 'person2Dob'),
    field('Gender', s.person2Gender, 'person2Gender'),
    section('Person 3 Details'),
    field('First Name', s.person3FirstName, 'person3FirstName'),
    field('Middle Name', s.person3MiddleName, 'person3MiddleName'),
    field('Last Name', s.person3SurName, 'person3SurName'),
    field("Is the middle name father's / husband's name?", s.person3MiddleNameType, 'person3MiddleNameType'),
    field('Date of Birth', s.person3Dob, 'person3Dob'),
    field('Gender', s.person3Gender, 'person3Gender'),
    section('Contact Details'),
    field('WhatsApp Number', s.mobile, 'mobile'),
    field('Email ID', s.email, 'email'),
    field('City of Birth', s.city, 'city'),
...
// Blueprint" — both save an identical field set; the add-ons each package
// offers already come through inside nameOptions' "[ADD-ON: ...]" tag(s),
// so no separate template is needed.
//
// FIX (2026-08-09): this was previously only registered under the
// packageType "single". A real order for "Complete Baby Name Blueprint"
// (₹6,694, add-on selected) came in with packageType "premium" instead, so
// the lookup in getOrderFormRows() missed this template entirely and fell
// back to the generic auto-formatter — which doesn't know this form's
// person-vs-child field shape, so it dumped duplicate/mislabeled rows like
// "Person 1 — Date of Birth" and "Person 1 — Name" (actually the father's
// name) onto the customer's invoice/email alongside the correct "Child's
// Date of Birth" row. Registered under both known packageType values now,
// plus a shape-based fallback below so any future packageType string for
// this same form (the exact set of values Empower's checkout may send
// isn't documented anywhere in this repo) still renders correctly instead
// of silently falling through to the generic formatter again.
function babyNameReportTemplate(s: Record<string, unknown>): FormRow[] {
  ...
  return [
    ...
    field('WhatsApp Number', s.mobile, 'mobile'),
    field('Email ID', s.email, 'email'),
    // FIX (2026-08-09): confirmed against ankshaastra-empower's own
    // OrderFormSection.tsx — the Baby Name Report form's submit payload has
    // NO "city" key at all; its "City of Birth" field is saved under
    // "placeOfBirth" instead (unlike the Name Check form, which really
    // does use "city"). This was reading the wrong key, so "City of Birth"
    // was rendering blank ("-") on every Baby Name Report invoice/email.
    field('City of Birth', s.placeOfBirth, 'placeOfBirth'),
    field('State/Province', s.state ?? s.customerState, 'state'),
    field('PIN Code', s.pinCode ?? s.pincode, 'pinCode'),
    field("Parents Profession", s.parentsProfession ?? s.parentProfession, 'parentsProfession'),
  ];
}
// Known packageType values seen for this form so far.
SERVICE_TEMPLATES['empower.ankshaastra.com::single'] = babyNameReportTemplate;
SERVICE_TEMPLATES['empower.ankshaastra.com::premium'] = babyNameReportTemplate;
// Also match on the human-readable service title, in case a future order
// carries that instead of/alongside packageType.
SERVICE_TEMPLATES['empower.ankshaastra.com::Perfect Baby Name Report'] = babyNameReportTemplate;
SERVICE_TEMPLATES['empower.ankshaastra.com::Complete Baby Name Blueprint'] = babyNameReportTemplate;
...
export function formatServiceDisplayName(order: Record<string, unknown>): string {
  ...
  // Empower's Baby Name Report packages (Perfect Baby Name Report /
  // Complete Baby Name Blueprint) — inferred (2026-08-09) from a real
  // order: packageType "premium" (₹6,694, one add-on) matches Complete
  // Baby Name Blueprint's ₹4,967 base + add-on pricing, while the lower
  // "single" tier matches Perfect Baby Name Report's ₹2,447 base. Only
  // applied when the form itself is shaped like the baby-name form
  // (childDob/fatherFullName present), so this can't mislabel some other,
  // unrelated service that happens to reuse "single"/"premium" as its own
  // packageType. TODO: confirm with client/Empower that these two exact
  // packageType strings are right for both packages — this is inferred
  // from one order, not from a documented list of values.
  if (snapshot.childDob || snapshot.fatherFullName) {
    if (packageType === 'single') return 'Perfect Baby Name Report';
    if (packageType === 'premium') return 'Complete Baby Name Blueprint';
  }
  return rawTitle;
}
...
export function getOrderFormRows(order: Record<string, unknown>): FormRow[] {
  ...
  // Shape-based fallback: Empower's Baby Name Report form (Perfect Baby
  // Name Report / Complete Baby Name Blueprint) is identifiable by its
  // distinctive fields (childDob/fatherFullName) regardless of whatever
  // packageType/service_title string this particular order happened to
  // carry — see the 2026-08-09 fix note above babyNameReportTemplate().
  if (sourceWebsite === 'empower.ankshaastra.com' && (formSnapshot.childDob || formSnapshot.fatherFullName)) {
    return babyNameReportTemplate(formSnapshot);
  }

  // Same hardening as above, for Miracle Baby: its template is currently
  // only keyed by the exact order.service_title strings "Complete Package"
  // / "Essentials Package". If a real order ever carries a differently-
  // worded or differently-cased title, it would silently fall through to
  // the generic formatter exactly like the Empower "premium" case did —
  // this shape-based check (mother/father/hospital fields are unique to
  // this form) catches that before it happens.
  if (sourceWebsite === 'miraclebaby.ankshaastra.com' && (formSnapshot.motherName || formSnapshot.hospitalName)) {
    return miracleBabyTemplate(formSnapshot);
  }
