...
function sectionTable(rows: Array<[string, string]>): string {
  ...
  const tableRows = rows
    .map(
      ([label, value]) =>
        `<tr><td style="padding:6px 12px 6px 0;font-weight:600;vertical-align:top;color:#444;white-space:nowrap;">${escapeHtml(label)}</td><td style="padding:6px 0;vertical-align:top;">${escapeHtml(value)}</td></tr>`,
    )
    .join('');
  return `<table style="border-collapse:collapse;width:100%;max-width:560px;font-size:14px;line-height:1.5;background:#f8fafc;border:1px solid #d9dee7;border-radius:8px;padding:4px 12px;margin-bottom:20px;">${tableRows}</table>`;
}
