...
import { getUserFromAuthHeader, hasModuleAccess } from '../lib/auth-api.js';
import { downloadInvoicePdfBuffer } from '../lib/invoice-storage.js';
import { regenerateInvoicePdfBuffer } from '../lib/invoice-regenerate.js';
...
export default async function handler(req: Req, res: Res) {
  ...
  try {
    ...
    const { data: invoice, error } = await supabase
      .from('invoices')
      .select('id, invoice_number, pdf_storage_path, pdf_purged_at')
      .eq('id', invoiceId)
      .single();
    ...
    if (invoice.pdf_storage_path) {
      buffer = await downloadInvoicePdfBuffer(invoice.pdf_storage_path);
    }

    // Stored file missing (never generated) or already purged by the
    // 6-month retention job (server/lib/invoice-pdf-retention.ts) — rebuild
    // it on demand from the permanent invoice/order records instead of
    // failing. Not written back to Storage, so this never re-fills it.
    if (!buffer) {
      buffer = await regenerateInvoicePdfBuffer(invoice.id);
      regenerated = true;
    }

    if (!buffer) {
      return res.status(404).json({ error: 'Could not produce invoice PDF' });
    }
    ...
    res.setHeader?.('Cache-Control', 'private, max-age=60');
    if (regenerated) {
      res.setHeader?.('X-Invoice-Regenerated', 'true');
    }
