...
// // Google Apps Script Web App — receives { rows: [...] } via POST and
// // writes them into the connected Google Sheet (replacing old rows each
// // time so the sheet always reflects the current abandoned-cart list).
// const GOOGLE_SHEET_SYNC_URL =
//   'https://script.google.com/macros/s/AKfycby0uRIP3IG_dAwULieBsGNxv74g3ffOr4IQdmU2F2_qIVJadWZAAaBmak40AIWn_y2x/exec';
...
//   // Push the current list straight into the Google Sheet — replaces the
//   // sheet's old rows each time, so it always shows exactly today's
//   // abandoned-cart list (no CSV download needed).
//   let sheetSynced = false;
//   try {
//     const sheetRes = await fetch(GOOGLE_SHEET_SYNC_URL, {
//       method: 'POST',
//       headers: { 'Content-Type': 'application/json' },
//       body: JSON.stringify({ rows }),
//     });
//     const sheetBody = await sheetRes.json().catch(() => ({}));
//     sheetSynced = sheetRes.ok && sheetBody.ok !== false;
//     if (!sheetSynced) errors.push(`Google Sheet sync failed: ${sheetBody.error || sheetRes.status}`);
//   } catch (e: unknown) {
//     errors.push(`Google Sheet sync: ${e instanceof Error ? e.message : String(e)}`);
//   }

//   return res.status(200).json({ count: rows.length, orders: rows, errors, sheetSynced });
// }
...
export default async function handler(req: Req, res: Res) {
  ...
  // Two sources are merged above (hub orders were sorted, Empower's were
  // not), so re-sort the combined list here — newest first — before it
  // goes anywhere (CSV export or the sheet sync below).
