...
export default async function handler(req: Req, res: Res) {
  const authHeader = req.headers?.authorization || req.headers?.Authorization;
  const user = await getUserFromAuthHeader(authHeader);
  // FIX: was isAdminUser(user.id) — ignored Team Management's per-module
  // permissions. hasModuleAccess() allows a full admin/super-admin OR a
  // user with an explicit can_view=true grant for the 'gst-maintenance'
  // module.
  if (!user || !(await hasModuleAccess(user.id, 'gst-maintenance'))) {
    return res.status(403).json({ error: 'Admin access required' });
  }
