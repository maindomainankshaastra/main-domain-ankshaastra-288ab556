...
export default async function handler(req: Req, res: Res) {
  ...
  try {
    ...
    return res.status(201).json({
      ...
      invoice_number: result.invoiceNumber,
      duplicate: result.duplicate ?? false,
      // TEMPORARY: step-by-step timing breakdown, shown directly in the
      // admin UI so the slow step can be found without digging through
      // Vercel logs. Remove once the slow step is found and fixed.
      timings: (result as { timings?: Record<string, number> }).timings,
    });
  } catch (e: unknown) {
    const msg = e instanceof Error ? e.message : 'Invoice creation failed';
    console.error('[invoices-create-manual]', msg, e);
