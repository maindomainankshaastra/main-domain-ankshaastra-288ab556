...
const PaymentPage = () => {
  ...
  // Generated once per visit and reused for BOTH the early auto-save
  // (below) and the final create-order call — so the real order UPDATEs
  // this same row instead of creating a duplicate one.
  const orderIdRef = useRef<string | null>(null);
  const getOrGenerateOrderId = () => {
  ...
  // Auto-save while the customer is still filling the form — no button
  // click needed. Debounced so it doesn't fire on every keystroke: waits
  // until they pause for a moment, and only once email or whatsapp (a real
  // way to reach them) has been entered. Best-effort: a failed save here
  // never blocks the form or the eventual real payment.
  const autoSaveTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  useEffect(() => {
    const sub = form.watch((value) => {
      const email = String((value as any)?.email || "");
      const whatsapp = String((value as any)?.whatsapp || "");
      ...
      autoSaveTimerRef.current = setTimeout(() => {
        const displayPersonName = resolvePurchaserName(value as Record<string, unknown>);
        fetch("/api/save-form-progress", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            ...
            sourceWebsite: "ankshaastra.com",
            customerName: displayPersonName,
            customerEmail: (value as any)?.email,
            customerPhone: (value as any)?.whatsapp,
            metadata: {
              formType,
              serviceSlug: serviceName,
              serviceId: serviceInfo?.id,
          ...
          /* best-effort only — the real order still saves at final submit */
        });
      }, 2500);
    });
  ...
  }, [form, formType, isServiceMode, serviceName, currentPackage, serviceInfo]);
  ...
  const handlePayment = async (amount: number, formData: any) => {
    ...
    try {
      const response = await fetch("/api/create-order", {
        ...
        body: JSON.stringify({
          amount,
          // Reuses the SAME id the auto-save above already used (if it
          // fired) — create-order.ts UPDATEs that row instead of creating
          // a second one for the same customer.
          existingOrderId: getOrGenerateOrderId(),
          serviceTitle: isServiceMode ? serviceName : currentPackage?.name || "Consultation",
          sourceWebsite: "ankshaastra.com",
          orderType: isServiceMode ? "service" : "consultation",
          userId: user?.id,
