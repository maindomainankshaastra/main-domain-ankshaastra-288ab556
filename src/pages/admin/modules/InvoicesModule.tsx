...
//   invoiceDate: string;
//   notes: string;
//   // Billing details — needed for a correct GST invoice (place of supply,
//   // CGST/SGST vs IGST) and so Address/GSTIN appear on the invoice instead
//   // of always showing "—".
//   customerCity: string;
...
//               {/* Billing details — required for correct GST invoicing
//                   (place of supply, CGST/SGST vs IGST) and so Address/GSTIN
//                   show up on the invoice instead of "—". City/Pincode are
//                   free text; State is a dropdown so the backend can reliably
//                   match it to a GST state code. GSTIN is optional — leave
//                   blank for a regular B2C consumer invoice. */}
//               <div className="grid grid-cols-2 gap-3">
//                 <div className="space-y-1.5">
...
//               <div className="grid grid-cols-2 gap-3">
//                 <div className="space-y-1.5">
//                   <Label htmlFor="inv-pincode">Pincode</Label>
//                   <Input
//                     id="inv-pincode"
//                     value={createForm.customerPincode}
//                     onChange={(e) => updateCreateForm("customerPincode", e.target.value)}
...
//               <p className="text-xs text-muted-foreground">
//                 This is an estimated preview. The final GST amount (and CGST/SGST vs IGST split) is
//                 calculated by the system based on the business's GST configuration and the customer's
//                 state at the time the invoice is generated.
//               </p>
...
//   // Field-level error messages only render after this becomes true, so the
//   // form doesn't show red errors before the user has had a chance to type.
...
//   // Required: customer name, email, service, a valid price, and billing
//   // details (city/state/pincode) — the latter three are what let the
//   // backend resolve place of supply correctly (CGST/SGST vs IGST) instead
//   // of silently falling back to the business's own state. GSTIN stays
//   // optional since most manual invoices are B2C.
...
//   // Field-level error flags, only surfaced once the user has attempted to
//   // go to preview at least once (createFormTouched).
