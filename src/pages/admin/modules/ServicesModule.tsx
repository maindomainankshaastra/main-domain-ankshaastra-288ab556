...
//   // Deep-link support: Global Search sends ?open=<service id>. Instead of
//   // auto-opening a read-only dialog, scroll to and briefly highlight that
//   // exact row in the table/cards, so the admin can use any action on it
//   // (View, Edit, Duplicate, Delete) themselves — not just see a static popup.
//   const [highlightedId, setHighlightedId] = useState<string | null>(null);
//   useEffect(() => {
//     const openId = searchParams.get("open");
//     if (!openId || !services.length) return;
//     const match = services.find((s) => s.id === openId);
//     if (match) {
//       setHighlightedId(openId);
//       setSearchParams({}, { replace: true });
//       setTimeout(() => {
//         document.getElementById(`service-row-${openId}`)?.scrollIntoView({
//           behavior: "smooth",
//           block: "center",
//         });
//       }, 100);
...
export default function ServicesModule() {
  ...
  // Deep-link support: Global Search sends ?open=<service id>. Instead of
  // auto-opening a read-only dialog, we clear any active filters (so the
  // record can't be hidden by them) and mark it to be highlighted once we
  // know which page it falls on (see the effect below sortedRows).
  const [highlightedId, setHighlightedId] = useState<string | null>(null);
  useEffect(() => {
    const openId = searchParams.get("open");
    if (!openId || !services.length) return;
  ...
  // Deep-link, step 2: once sortedRows reflects the full, unfiltered list
  // (filters were cleared above), find which page the target record falls
  // on and jump there — otherwise it may be on page 2/3/etc and never
  // render for scrollIntoView to find.
  useEffect(() => {
    if (!highlightedId) return;
    const index = sortedRows.findIndex((s) => s.id === highlightedId);
    if (index === -1) return;
    const targetPage = Math.floor(index / PAGE_SIZE) + 1;
    setCurrentPage(targetPage);
    setTimeout(() => {
      document.getElementById(`service-row-${highlightedId}`)?.scrollIntoView({
        behavior: "smooth",
        block: "center",
      });
    }, 150);
                    ...
                    {paginatedRows.map((s) => (
                      <tr
                        ...
                        className={cn(
                          ...
                          highlightedId === s.id
                            ? "border-primary ring-2 ring-inset ring-primary/40 bg-primary/5"
                            : "border-border"
                        )}
                      >
                        <td className="px-4 py-2 font-medium truncate max-w-[220px]">{s.title}</td>
              ...
              <div className="md:hidden space-y-3">
                {paginatedRows.map((s) => (
                  <div
                    ...
                    onClick={() => setViewing(s)}
                    className={cn(
                      "rounded-lg border p-3 space-y-2 active:bg-muted/40 transition-colors",
                      highlightedId === s.id
