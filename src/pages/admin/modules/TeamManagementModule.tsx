...
const TeamManagementModule = () => {
  ...
  // Removal now does two things: revokes current access (unchanged), and
  // permanently records the removal so this user can never be found in
  // search or re-added again — only a database administrator can undo
  // that by deleting their row from staff_removals.
  const performRemoveStaff = async () => {
    if (!removeTarget) return;
    setRemoving(true);
    await supabase.from("admin_module_permissions").delete().eq("user_id", removeTarget.user_id);
    await supabase.from("user_roles").delete().eq("user_id", removeTarget.user_id).eq("role", "staff");
    await supabase.from("staff_removals").upsert({ user_id: removeTarget.user_id });
    setRemoving(false);
    toast.success("Staff access removed");
    setRemoveTarget(null);
    loadStaffList();
