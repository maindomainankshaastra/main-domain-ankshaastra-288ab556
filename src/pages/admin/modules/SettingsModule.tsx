...
// Fixed business rules — must match server/lib/gst-auto-fix.ts exactly.
// GST Maintenance always uses these two constants regardless of what is
// stored in gst_config, so these fields are readonly here to avoid the
// false impression that changing them would affect historical GST repair.
const FIXED_SAC_CODE = "999799";
...
export default function SettingsModule() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const config = await loadGstConfig(token);
        // Always display (and, on next save, persist) the fixed values here
        // — whatever was previously saved in the DB for these two fields no
        // longer has any effect and shouldn't be shown as if it does.
        setForm({ ...config, default_sac_code: FIXED_SAC_CODE, default_gst_rate: FIXED_GST_RATE });
      } catch (e: unknown) {
        toast.error(e instanceof Error ? e.message : "Could not load GST settings");
      } finally {
        setLoading(false);
  ...
  const save = async () => {
    ...
    try {
      ...
      // Belt-and-braces: force the fixed values into the saved payload too,
      // independent of whatever is in local form state.
      await saveGstConfig(token, { ...form, default_sac_code: FIXED_SAC_CODE, default_gst_rate: FIXED_GST_RATE });
      const config = await loadGstConfig(token);
      setForm({ ...config, default_sac_code: FIXED_SAC_CODE, default_gst_rate: FIXED_GST_RATE });
      toast.success("GST settings saved");
    } catch (e: unknown) {
    ...
    <AdminPage title="GST Configuration" description="Legal entity, SAC codes, invoice numbering, bank details, and GSTR filing settings." loading={loading}>
      <div className="grid gap-4 max-w-lg">
        ...
        <div>
          ...
          <Input value={FIXED_SAC_CODE} disabled readOnly className="bg-muted cursor-not-allowed" />
          <p className="text-xs text-muted-foreground mt-1">Fixed business rule — always {FIXED_SAC_CODE}. Cannot be edited here; GST Maintenance always uses this value regardless of what was previously saved.</p>
        </div>
        <div>
          <Label>Default GST Rate %</Label>
          <Input type="number" value={FIXED_GST_RATE} disabled readOnly className="bg-muted cursor-not-allowed" />
          <p className="text-xs text-muted-foreground mt-1">Fixed business rule — always {FIXED_GST_RATE}%. Cannot be edited here; GST Maintenance always uses this value regardless of what was previously saved.</p>
        </div>
        <div><Label>Invoice Prefix</Label><Input value={String(form.invoice_prefix || "INV")} onChange={(e) => setForm({ ...form, invoice_prefix: e.target.value })} placeholder="e.g. EYN26-27K-7000" /><p className="text-xs text-muted-foreground mt-1">Invoice numbers are generated as prefix + sequence.</p></div>
        <div><Label>Financial Year</Label><Input value={String(form.financial_year || "")} onChange={(e) => setForm({ ...form, financial_year: e.target.value })} placeholder="e.g. 2025-26" /></div>
        <div><Label>GST Filing Frequency</Label><Input value={String(form.gst_filing_frequency || "monthly")} onChange={(e) => setForm({ ...form, gst_filing_frequency: e.target.value })} placeholder="monthly or quarterly" /></div>
