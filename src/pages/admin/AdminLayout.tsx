...
const navSections: NavSection[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { to: "/admin/reports", label: "Reports", icon: ScanSearch, module: "name-check-reports" },
      { to: "/admin/team", label: "Team Management", icon: UserCog, module: "team", adminOnly: true },
      // Audit Logs — accountability/security log. adminOnly: true means the
      // existing visibleSections filter below (role === "admin") already
      // hides this from Staff/Limited Admins/Team Members automatically —
      // no new logic needed, same mechanism already protecting Team
      // Management. The backend (admin-audit-logs.ts) independently
      // enforces the same check, so even a direct URL visit is blocked.
      { to: "/admin/audit-logs", label: "Audit Logs", icon: History, module: "audit-logs", adminOnly: true },
    ],
  },
];
...
type SearchResult = {
  ...
  // Which connected website this record belongs to (e.g. ankshaastra.com).
  // Falls back to "—" for records with no such concept (e.g. Team Members).
  website: string;
};
...
const WEBSITE_FIELD_CANDIDATES = ["source_website", "website", "site"];
      ...
      try {
        ...
        return data
          ...
          .map((row: Record<string, unknown>) => ({
            ...
            website: pickField(row, WEBSITE_FIELD_CANDIDATES) || "—",
          }));
      } catch (e) {
...
const AdminLayout = () => {
  ...
  const handleSelectResult = (result: SearchResult) => {
    ...
    setDebouncedQuery("");
    setGroupedResults({});
    // Deep-link: pass the specific record's id via ?open=, so the target
    // module can auto-open that exact record's existing View dialog
    // instead of just landing on the module's list page.
    navigate(`${result.route}?open=${encodeURIComponent(result.id)}`);
  };
  ...
  const hasAccessToCurrentPage =
    ...
    (!currentNavItem.adminOnly && canViewModule(currentNavItem.module));
                          ...
                          {(groupedResults[source.module] || []).map((result) => {
                            ...
                            return (
                              ...
                              >
                                ...
                                <span className="flex flex-col items-end text-xs text-muted-foreground shrink-0 gap-0.5">
                                  <span>{result.website}</span>
                                  <span>{formatResultDate(result.createdAt)}</span>
                                </span>
                              </button>
                            );
                          })}
