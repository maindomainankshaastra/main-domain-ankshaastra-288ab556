...
function drawCurrentNameBreakdownPage(
  ...
  if (opts.reducedTo !== undefined) {
    ...
    page.drawText(label, {
      x: boxX + half / 2 - fonts.sansBold.widthOfTextAtSize(label, labelSize) / 2,
      y: totalsY - totalsRowH / 2 - labelSize * 0.35,
      size: labelSize,
      font: fonts.sansBold,
      color: COLOR.maroonDark,
    });
    page.drawText(value, {
      x: boxX + half + half / 2 - fonts.sansBold.widthOfTextAtSize(value, valueSize) / 2,
      y: totalsY - totalsRowH / 2 - valueSize * 0.35,
      size: valueSize,
      font: fonts.sansBold,
      color: COLOR.maroonDark,
    });
    page.drawLine({ start: { x: boxX + half, y: totalsY }, end: { x: boxX + half, y: totalsY - totalsRowH }, thickness: 0.5, color: rgb(0.82, 0.68, 0.66) });
  }
