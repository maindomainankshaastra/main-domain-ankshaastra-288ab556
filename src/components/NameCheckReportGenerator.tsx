...
const TYPE = {
  ...
  lineGap: 5, // extra px added to font size for line-height inside a paragraph
  bulletGap: 14, // vertical gap BETWEEN bullet items (was inconsistently 4/8/12/16 per page)
  bulletDotOffsetRatio: 0.32, // dot's y-offset above the text baseline, as a fraction of font size
  cardPadding: 46, // standard top padding inside a rounded content box, banner to first line of text
};
...
function drawBulletList(
  ...
  let y = opts.y;
  // Dot sits ABOVE the text baseline, roughly at the visual mid-height of the
  // line — a POSITIVE offset. (A negative offset here puts the dot below the
  // baseline, which is what makes bullets look disconnected/sunk beneath
  // their text — that was the actual bug, not the fixed-vs-relative offset.)
  const dotY = opts.size * TYPE.bulletDotOffsetRatio;
  items.forEach((item) => {
    page.drawCircle({ x: opts.x + 3, y: y + dotY, size: 2.4, color: COLOR.maroon });
    y = drawWrappedText(page, item, {
...
function drawCoverPage(page: PDFPage, fonts: Fonts, data: Required<NameCheckReportInput>, assets: Assets) {
  ...
  const LOGO_TARGET_WIDTH = px(890);
  const logoScale = LOGO_TARGET_WIDTH / assets.logo.width;
  const logoDims = assets.logo.scale(logoScale);
  let y = PAGE_HEIGHT - 46;
  page.drawImage(assets.logo, { x: centerX - logoDims.width / 2, y: y - logoDims.height, width: logoDims.width, height: logoDims.height });
  y -= logoDims.height + 66;

  const GRID_TARGET_WIDTH = px(940);
  const gridScale = GRID_TARGET_WIDTH / assets.loshuGrid.width;
  const gridDims = assets.loshuGrid.scale(gridScale);
  page.drawImage(assets.loshuGrid, { x: centerX - gridDims.width / 2, y: y - gridDims.height, width: gridDims.width, height: gridDims.height });
  y -= gridDims.height + 60;
  ...
  y -= bylineSize * 0.42;
  page.drawLine({ start: { x: centerX - 165, y }, end: { x: centerX - 14, y }, thickness: 1, color: COLOR.maroon });
  page.drawLine({ start: { x: centerX + 14, y }, end: { x: centerX + 165, y }, thickness: 1, color: COLOR.maroon });
  drawStarGlyph(page, centerX, y, 6, COLOR.maroon);
...
function drawBlueprintPage(
  ...
  const bannerY = dividerY - 90;
  const bannerHeight = bannerSize + 26;
  drawRoundedRect(page, { x: boxX, y: bannerY - bannerHeight, width: boxWidth, height: bannerHeight, radius: 16, color: COLOR.maroon });
  ...
  // The table's row height is derived from the space actually available
  // between the banner and the footer pill, so the table fills the page the
  // way the Bindhu reference does — instead of a fixed small row height that
  // left a large empty gap above the footer.
  const tableTop = bannerY - bannerHeight - 12;
  const footerSafeTop = 90; // keep clear of the footer pill
  const computedRowHeight = Math.min(60, Math.max(38, (tableTop - footerSafeTop) / rows.length));
  drawDataTable(page, fonts, rows, { x: boxX, y: tableTop, width: boxWidth, rowHeight: computedRowHeight });
}
...
function drawCurrentNameBreakdownPage(
  ...
  const nameRowH = 44;
  const totalsRowH = 50;
  const gapBetween = 10; // visible gap between the two standalone boxes, matching the reference
  let y = PAGE_HEIGHT - 215;

  // Box 1: standalone fully-rounded maroon pill — "First Name | Priyanka".
  // This must NOT be fused with the totals box below it (that was the "no
  // dibba/box" bug): the reference clearly renders these as two separate
  // floating rounded rectangles with a gap between them, not one continuous
  // pill with a flat seam in the middle.
  ...
  // Box 2: standalone fully-rounded outlined box — "Total | Reduced To".
...
function drawWhyCriticalPage(
  ...
  // NOTE: no `subtitle` here. The reference renders this section with ONE
  // "Why This Is Critical" label — inside the maroon banner below — not a
  // second plain-text copy above it. Passing subtitle here (as a previous
  // version of this file did) draws BOTH, producing a visibly duplicated
  // heading.
  drawPageChrome(page, fonts, assets, { title: "Current Name\nBreakdown", pageNumber, totalPages, brand: data.brand });
  ...
  const bodySize = TYPE.bodySize;
  const lineHeight = bodySize + TYPE.lineGap + 0.5;
  // Without the subtitle line, content can sit closer to the divider than the
  // other Current-Name-Breakdown pages (which reserve space for their
  // subtitle) — 193 (vs. 215 elsewhere) removes exactly that reserved gap.
  const boxTop = PAGE_HEIGHT - 193;
  const innerWidth = boxWidth - 36;
...
/**
 * Page 14 — services list. Matches the reference's layout: centered logo at
 * top, a single small-caps title line, a divider, the "SERVICES OFFERED"
 * maroon pill banner, then the bulleted service list — NOT the two-tier
 * drawPageChrome title treatment used on the analytical pages, and not
 * missing the logo/banner the way an earlier version of this function did.
 */
function drawServicesPage(page: PDFPage, fonts: Fonts, assets: Assets, data: Required<NameCheckReportInput>, pageNumber: number, totalPages: number) {
  drawPageBackground(page, assets);
  const centerX = PAGE_WIDTH / 2;
  ...
  const titleSize = px(45);
  const title = "ILLUMINATING LIVES THROUGH ANCIENT WISDOM";
  page.drawText(title, { x: centerX - fonts.heading.widthOfTextAtSize(title, titleSize) / 2, y, size: titleSize, font: fonts.heading, color: COLOR.maroonDark });
  y -= titleSize + 14;
  ...
  const bannerSize = px(45);
  const bannerLabel = "SERVICES OFFERED";
  const bannerWidth = fonts.sansBold.widthOfTextAtSize(bannerLabel, bannerSize) + 56;
  const bannerHeight = bannerSize + 24;
  const bannerY = dividerY - 30;
  drawRoundedRect(page, { x: centerX - bannerWidth / 2, y: bannerY - bannerHeight, width: bannerWidth, height: bannerHeight, radius: bannerHeight / 2, color: COLOR.maroon });
  page.drawText(bannerLabel, {
    x: centerX - fonts.sansBold.widthOfTextAtSize(bannerLabel, bannerSize) / 2,
    y: bannerY - bannerHeight / 2 - bannerSize * 0.35,
    size: bannerSize,
    font: fonts.sansBold,
    color: COLOR.white,
  ...
  let listY = bannerY - bannerHeight - 46;
  const dotR = 9;
  const bodySize = px(45);
  services.forEach((service) => {
    page.drawCircle({ x: 44 + dotR, y: listY - 5, size: dotR, borderColor: COLOR.maroon, borderWidth: 1 });
    drawStarGlyph(page, 44 + dotR, listY - 5, 3.6, COLOR.maroon);
    page.drawText(service, { x: 44 + dotR * 2 + 14, y: listY - bodySize * 0.35 - 5, size: bodySize, font: fonts.sans, color: COLOR.ink });
    listY -= bodySize + 20;
  });
}
...
function drawConnectPage(page: PDFPage, fonts: Fonts, assets: Assets, data: Required<NameCheckReportInput>, pageNumber: number, totalPages: number) {
  ...
  const colXs = [centerX - 155, centerX + 155];
  // Row gap and icon/button sizes were oversized relative to the Bindhu
  // reference — measured against the reference at matching scale, the
  // circle diameter here needs to shrink roughly 15%, and the vertical
  // rhythm between rows / the closing text tightens to match.
  const rowYs = [dividerY - subtitleSize - 95, dividerY - subtitleSize - 200];
  let idx = 0;
  rowYs.forEach((ry) => {
    colXs.forEach((cx) => {
      const { image, url } = platforms[idx++];
      ...
      page.drawCircle({ x: cx, y: ry, size: 34, borderColor: COLOR.maroon, borderWidth: 1.1 });
      page.drawCircle({ x: cx, y: ry, size: 29, color: COLOR.maroon });
      const iconW = 25;
      const iconH = (image.height / image.width) * iconW;
      page.drawImage(image, { x: cx - iconW / 2, y: ry - iconH / 2, width: iconW, height: iconH });
      ...
      const btnH = 23;
      const btnTopY = ry - 46;
      drawRoundedRect(page, { x: cx - btnW / 2, y: btnTopY, width: btnW, height: btnH, radius: btnH / 2, color: COLOR.maroon });
      const btnLabel = "CLICK ME";
      const btnLabelSize = 8.5;
      const btnLabelWidth = fonts.sansBold.widthOfTextAtSize(btnLabel, btnLabelSize);
      const btnLabelX = cx - btnLabelWidth / 2;
      const btnLabelY = btnTopY + btnH / 2 - 3;
      page.drawText(btnLabel, { x: btnLabelX, y: btnLabelY, size: btnLabelSize, font: fonts.sansBold, color: COLOR.white });
      ...
      addLinkAnnotation(page, url, { x: cx - btnW / 2, y: btnTopY, width: btnW, height: ry + 34 - btnTopY });
    });
  });

  const closingSize = px(60);
  const closingY = rowYs[1] - 75;
  ["STAY CONNECTED FOR", "ONGOING GUIDANCE & SUPPORT"].forEach((line, i) => {
    page.drawText(line, { x: centerX - fonts.sansBold.widthOfTextAtSize(line, closingSize) / 2, y: closingY - i * (closingSize + 6), size: closingSize, font: fonts.sansBold, color: COLOR.maroonDark });
  });
}
