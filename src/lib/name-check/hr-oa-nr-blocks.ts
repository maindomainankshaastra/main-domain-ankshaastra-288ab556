...
/**
 * HR / OA / NR RULE TEXT BLOCKS
 * Exact wording transcribed from the client's UPDATED "Name Check Output"
 * doc (Himansshu Agarwal, Ankshaastra) — the 18-rule revision that
 * replaces the original 39-rule version.
 *
 * Each rule also carries a short `condition` description (in comments,
 * taken directly from the rule's docx heading) — this is what the actual
 * matching logic in rule-engine.ts checks for. The condition is NOT
 * auto-evaluated here; this file is text + metadata only.
 *
 * ============================================================================
 * CHANGE LOG — old 39-rule doc -> new 18-rule doc (client confirmed)
 * ============================================================================
 *  - Rule count cut from 39 (22 HR + 13 OA + 4 NR) down to 18
 *    (11 HR + 4 OA + 3 NR). All removed rules are listed at the bottom of
 *    this comment for audit purposes.
 *  - VERDICT CHANGE: First Name (Enemy) + Full Name (Friendly) was OA-01
 *    (Optional) in the old doc. In the new doc this is HR-05 (Highly
 *    Recommended) — a genuine verdict upgrade, not just a renumber.
 *  - HR-11 (Rajyog Potential) — previously HR-20 in the old doc, and
 *    explicitly SKIPPED/not implemented per an earlier client instruction.
 *    Client has now reversed that and confirmed the exact detection
 *    logic (see lo-shu.ts -> getRajyogPotential()): a Rajyog Potential
 *    exists in the birth chart's Lo Shu grid when both flanking numbers
 *    of 9-5-1, 2-5-8, or 4-5-6 are present and 5 itself is missing.
 *    Combined with First Name Friendly + Full Name Friendly, this is now
 *    ACTIVE and implemented in rule-engine.ts.
 *  - CONFIRMED: for First Name (Neutral) or First Name (Enemy) combos,
 *    the compound number tier no longer changes the verdict — these
 *    combos resolve to a single fixed rule regardless of compound. (This
 *    is why all the old compound-stacked variants for these combos —
 *    HR-15, HR-17, HR-19, OA-05, OA-06 — were dropped.)
 *  - CONFIRMED: restricted numbers (4/8/9) always resolve to the plain
 *    HR-07/08/09 rule, even when BOTH first and full name land on a
 *    restricted number (same or different). No more special dual-name
 *    restricted-number rules (old HR-10, HR-11, HR-12, HR-13, HR-21 all
 *    dropped as a result).
 *  - CONFIRMED: Lo Shu "repetition/over-amplification" is NOT checked
 *    for verdict matching anymore — only plain Friendly/Neutral/Enemy
 *    status matters (except for the new, separately-defined Rajyog
 *    Potential check above, which is a different present/missing check,
 *    not a repetition check). This is why old HR-05, HR-06, OA-08, OA-09
 *    were dropped.
 *  - CONFIRMED: "Name Number equals Mulank/Bhagyank" is no longer a
 *    tracked condition — old OA-12, OA-13 dropped.
 *  - CONFIRMED: the "nothing matched" fallback should essentially never
 *    occur with the new rule set (one known theoretical gap: First Name
 *    Friendly + Full Name Friendly + Moderate/Neutral compound, with no
 *    weak-friendly and no Rajyog potential, has no defined rule in the
 *    new doc). If it's ever hit, rule-engine.ts logs a console.warn for
 *    internal/admin visibility only — nothing is shown to the end
 *    customer, and the existing admin-correction workflow (see
 *    NameCheckReportsModule.tsx) is where a human fills in the content
 *    manually before the report goes out.
 *
 *  RULES REMOVED ENTIRELY (present in old 39-rule doc, absent from new
 *  18-rule doc): HR-05, HR-06, HR-10, HR-11, HR-12, HR-13, HR-15, HR-17,
 *  HR-18, HR-19, HR-21, HR-22, OA-05, OA-06, OA-07, OA-08, OA-09, OA-10,
 *  OA-12, OA-13, and the old NR-03 (First Name Neutral + Full Name
 *  Friendly + Good Compound).
 *
 *  RULES KEPT BUT RENUMBERED (text/verdict unchanged unless noted):
 *    old HR-14 -> new HR-06 (F-F + Bad Compound)
 *    old HR-16 -> new HR-10 (F-F Friendly & Weak)
 *    old HR-20 -> new HR-11 (Rajyog Potential — now ACTIVE, see above)
 *    old OA-01 -> new HR-05 (E-F — VERDICT CHANGED OA -> HR, text tweaked)
 *    old OA-02 -> new OA-01 (N-N)
 *    old OA-03 -> new OA-02 (F-N)
 *    old OA-04 -> new OA-03 (N-F)
 *    old OA-11 -> new OA-04 (Compound Conditional, standalone)
 *    old NR-04 -> new NR-03 (F-F + Conditional Compound)
 *  RULES UNCHANGED (id, text, verdict all the same):
 *    HR-01, HR-02, HR-03, HR-04, HR-07, HR-08, HR-09, NR-01, NR-02
 * ============================================================================
 */
...
export const HR_RULES: RuleBlock[] = [
  ...
  },
  {
    id: "HR-05",
    verdict: "HR",
    conditionLabel: "First Name (Enemy) + Full Name (Friendly) — VERDICT UPGRADED from old OA-01",
    paragraphs: [
      "Your full name is well-aligned and actively supports your life path and destiny, the most important layer working well, but your first name creates daily friction.",
      "This creates a subtle disconnect, where your broader life path is supported, but daily personal expression, self-confidence, and everyday interactions are consistently affected by this conflict.",
  ...
  },
  {
    id: "HR-11",
    verdict: "HR",
    conditionLabel: "First Name (Friendly) + Full Name (Friendly, Rajyog Potential) — CONFIRMED (2nd round): birth chart Lo Shu grid has 9&1, 2&8, or 4&6 present with 5 missing",
    paragraphs: [
      "Your first name vibrates on a friendly number aligned with your birth energies, and your full name also carries a friendly vibration, a stable, supportive nameprint overall.",
      "However, the full name number holds the potential to form a Rajyog combination, one of the most powerful configurations in numerology, built for accelerated growth and success.",
      "Name correction is recommended. The full name should ideally be adjusted to fully activate the Rajyog, so a good nameprint becomes a genuinely exceptional one instead.",
...
export const OA_RULES: RuleBlock[] = [
  ...
  },
  {
    id: "OA-04",
    verdict: "OA",
    conditionLabel: "Compound Number is Conditional (standalone — see rule-engine.ts note: this may be unreachable given full Friendly/Neutral/Enemy grid coverage above; flagged for client review)",
    paragraphs: [
      "Your name carries a conditional compound, genuine positive potential that comes alongside a specific area of caution or risk requiring conscious acknowledgment and active management here.",
      "The positive and cautionary elements exist together, so the associated vulnerability is real and, if ignored, tends to become a recurring pattern offsetting some of the gains.",
      "Correction to an unconditionally auspicious compound is the cleaner choice, though retaining this compound with full conscious awareness remains a meaningful alternative when alignment is strong.",
